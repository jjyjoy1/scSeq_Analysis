...
def train_dca(adata, batch_key='batch', latent_dim=32, hidden_dim=128, 
    ...
    """
    print(f"Training DCA with latent_dim={latent_dim}, hidden_dim={hidden_dim}, " +
          f"learning_rate={learning_rate}, use_zinb={use_zinb}")
    ...
    # Define custom loss function based on model outputs
    if use_zinb:
        def zinb_loss_fn(y_true, y_pred):
            # This function will be called with a list of outputs from the model
            # y_pred is not used here since we get the actual outputs from the model
            mean_output = model.get_layer('mean_activation').output
            disp_output = model.get_layer('dispersion_activation').output
            pi_output = model.get_layer('pi_activation').output
            return zinb_loss_separate(y_true, mean_output, disp_output, pi_output)
        
        loss_fn = zinb_loss_fn
    else:
        def nb_loss_fn(y_true, y_pred):
            # This function will be called with a list of outputs from the model
            # y_pred is not used here since we get the actual outputs from the model
            mean_output = model.get_layer('mean_activation').output
            disp_output = model.get_layer('dispersion_activation').output
            return zinb_loss_separate(y_true, mean_output, disp_output)
        
        loss_fn = nb_loss_fn
    
    # Compile model with the custom loss
    optimizer = optimizers.Adam(learning_rate=learning_rate)
    
    # Create a custom model with multiple outputs but single loss
    input_tensor = layers.Input(shape=(input_dim,))
    outputs = model(input_tensor)
    
    # Combine model outputs into single loss
    if use_zinb:
        # Create TensorFlow function to compute the loss
        @tf.function
        def compute_loss(y_true, mean, disp, pi):
            return zinb_loss_separate(y_true, mean, disp, pi)
        
        # Dummy output for compilation
        dummy_output = layers.Lambda(lambda x: x[0])(outputs)
        
        # Create custom training model
        training_model = Model(inputs=input_tensor, outputs=dummy_output)
        ...
        # Create TensorFlow function to compute the loss
        @tf.function
        def compute_loss(y_true, mean, disp):
            return zinb_loss_separate(y_true, mean, disp)
    ...
    # Compile the training model with mean squared error (not actually used)
    training_model.compile(optimizer=optimizer, loss='mse')
    ...
    for epoch in range(epochs):
        ...
        for batch in range(num_batches):
            ...
            batch_loss = train_step(x_batch, x_batch)
